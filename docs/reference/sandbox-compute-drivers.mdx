...
On Linux x86_64 PVM hosts, configure an external PVM guest kernel and qboot firmware:
...
PVM sandboxes and image-preparation VMs use QEMU and user-mode networking because PVM requires qboot's bootstrap transition. The driver adds the required `pti=off` kernel option. The kernel must include the virtio, filesystem, namespace, cgroup, seccomp, Landlock, bridge, and netfilter features required by the sandbox supervisor. `qemu-system-x86_64` must be available on `PATH`.
