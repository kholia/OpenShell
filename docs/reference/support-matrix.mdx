...
|---|---|---|
| Docker | Supported for local development and single-machine gateways. | Requires Docker Desktop or Docker Engine on the gateway host. Native Linux Docker Engine can use an explicitly configured alternate OCI runtime such as gVisor `runsc`. |
...
| gVisor `runsc` | Current release | Optional for Docker-backed sandboxes on native Linux. Install and register it with the same Docker daemon that OpenShell uses. |
